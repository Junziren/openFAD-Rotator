...
export class RotorEffects {
  ...
  get feedbackActive() { return this.enabled && this.feedbackAge < 1; }
  ...
  update(dt: number, direction: number, doppler: number, motion: number, bands: number[] = [0, 0, 0]) {
    ...
    const legacy = this.mode !== "spatial" && energy > .001;
    this.points.visible = this.trailLines.visible = legacy;
    (this.points.material as THREE.PointsMaterial).opacity = .38 * this.strength * energy;
    (this.trailLines.material as THREE.LineBasicMaterial).opacity = .22 * this.strength * energy;
    for (let i = 0; i < 36; i++) {
      const band = i % 3;
    ...
    this.rings.forEach((ring, i) => {
      ...
      (ring.material as THREE.MeshBasicMaterial).opacity = this.strength * (bands[Math.floor(i / 2)] ?? 0) * (ring.userData.halo ? .055 : .3);
    });
    this.waves.forEach((wave, i) => {
      wave.visible = this.mode !== "legacy";
      const age = ((Math.abs(this.clock) * .24 + i / 3) % 1);
      wave.scale.setScalar(1.1 + age * .8);
      (wave.material as THREE.MeshBasicMaterial).opacity = Math.sin(age * Math.PI) * .25 * this.strength * energy;
    });
