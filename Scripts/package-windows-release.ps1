...
foreach ($required in @($vst3, $standalone, $loader, $vst3Binary)) {
...
New-Item -ItemType Directory -Force -Path (Split-Path -Parent $stageVst3), $stageStandalone, $stageDocs | Out-Null
...
$metadata = [ordered]@{
    ...
    includes = @("VST3", "Standalone", "WebView2Loader", "Docs", "LICENSE", "THIRD_PARTY_NOTICES.md")
    sha256 = $hashes
}
