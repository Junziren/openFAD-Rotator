...
const required = ["id", "name", "description", "lowCut", "highCut", "lowGain", "midGain", "highGain"];
const ids = new Set();
for (const [index, profile] of manifest.profiles.entries()) {
  ...
  for (const key of ["lowCut", "highCut", "lowGain", "midGain", "highGain"]) {
  ...
  if ([profile.lowGain, profile.midGain, profile.highGain].some((value) => value < 0.1 || value > 4))
